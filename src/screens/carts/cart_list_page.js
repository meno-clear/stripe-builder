...
export default function CartListPage() {
  ...
  function refreshList() {
    api_client.get('/carts').then((response) => {
      setCarts(response.data);
    });
  }

  function deleteItem(item) {
    api_client.delete(`carts/${item}`).then(() => {
  ...
  const Item = ({ item }) => {
    return (
      <Swipeable renderLeftActions={() => getLeftContent(item)}>
        <TouchableOpacity
          style={styles.item}
          onPress={() => navigation.navigate('CartForm', { id: item.id })}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginBottom: 20, padding: 15 }}>
        ...
        <Text style={styles.headerTitle}> Carts List</Text>
        <TouchableOpacity style={styles.buttonToAdd} onPress={() => navigation.navigate('CartForm')}>
