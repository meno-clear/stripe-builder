...
export default function OrderListPage() {
  ...
  function deleteItem(item) {
    api_client.delete(`orders/${item}`).then(() => {
  ...
  const Item = ({ item }) => {
    return (
      <Swipeable renderLeftActions={() => getLeftContent(item)}>
        <TouchableOpacity
          style={styles.item}
          onPress={() => navigation.navigate('OrderForm', { id: item.id })}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginBottom: 20, padding: 15 }}>
        ...
        <TouchableOpacity style={styles.buttonToAdd} onPress={() => navigation.navigate('OrderForm')}>
