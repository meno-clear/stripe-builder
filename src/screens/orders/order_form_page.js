...
export default function OrderFormPage({route}) {
  ...
  useFocusEffect(
    useCallback(() => {
      if (route?.params?.id) {
        api_client.get(`orders/${route?.params?.id}`).then((response) => {
  ...
  const handleSubmit = () => {
    ...
    if (route?.params?.id) {
      api_client.put(`orders/${route?.params?.id}`, order).then((response) => {
      ...
      api_client.post('orders', order).then((response) => {
        if (response.data.errors) {
          setErrors(response.data.errors);
        } else {
          navigation.navigate('OrderList');
