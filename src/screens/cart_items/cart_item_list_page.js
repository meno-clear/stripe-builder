...
export default function CartItemListPage() {
  ...
  function refreshList() {
    api_client.get('/cart_items').then((response) => {
      setCartItems(response.data);
    });
  }

  function deleteItem(item) {
    api_client.delete(`cart_items/${item}`).then(() => {
  ...
  const Item = ({ item }) => {
    return (
      <Swipeable renderLeftActions={() => getLeftContent(item)}>
        <TouchableOpacity
          style={styles.item}
          onPress={() => navigation.navigate('CartItemForm', { id: item.id })}
        >
          
            <Text style={styles.title}>ProductName: <Text style={styles.data}> {item?.product_name}</Text></Text>
          
            <Text style={styles.title}>ProductPriceInCents: <Text style={styles.data}> {item?.product_price_in_cents}</Text></Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginBottom: 20, padding: 15 }}>
        ...
        <Text style={styles.headerTitle}> Cart Items List</Text>
        <TouchableOpacity style={styles.buttonToAdd} onPress={() => navigation.navigate('CartItemForm')}>
